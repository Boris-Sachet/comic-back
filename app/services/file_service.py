...
def create_file_model(file_path: str):
    name, extension = splitext(basename(file_path))
    pages_list = count_pages(file_path)
    file_dict = {
        "full_path": file_path,
...
async def execute_action(file: FileModel, action: str) -> FileModel:
    """Execute the given reading action on the given reading file"""
    match action:
        case "+": return await next_page(file)
        case "-": return await prev_page(file)
        case _:   return await set_page(file, int(action))
...
def get_full_path(file_path: str) -> str:
    ...
    return join(base_dir, file_path)


def calculate_md5(file_path: str) -> str:
    """Calculate md5 signature of a file"""
    return hashlib.md5(pathlib.Path(get_full_path(file_path)).read_bytes()).hexdigest()


async def get_file_from_db(file_path: str) -> FileModel:
    ...
    db_file = await find_file_by_full_path(file_path)
    if db_file:
        ...
        try:
            ...
            if not db_file:
                ...
                return await update_file(str(db_file.id), file_updated)
...
# Pages methods
def count_pages(file_path: str) -> List[str]:
    ...
    pages_names = []
    with opener_lib(get_full_path(file_path), 'r') as file:
...
def get_page(file_data: FileModel, num: int = 0):
    """Get a specific page with a given number"""
    opener_lib = get_opener_lib(file_data.extension)
    with opener_lib(get_full_path(file_data.full_path), 'r') as file:
        with file.open(file_data.pages_names[num]) as img:
            return img.read()


def get_current_page(file: FileModel):
    """Return file data corresponding to the current page number"""
    return get_page(file, file.current_page)


async def set_page(file: FileModel, num: int) -> FileModel:
    """Set the current page of a file in the database and return the updated FileModel object"""
    if 0 <= num <= file.pages_count - 1:
        return await update_file(str(file.id), UpdateFileModel(**{"current_page": num}))
    return file


async def next_page(file: FileModel) -> FileModel:
    """Increment current page for file"""
    return await set_page(file, file.current_page + 1)


async def prev_page(file: FileModel) -> FileModel:
